...
- Add passenger names while booking seats.
- Mark rides completed or delete old rides.
- Copy a selected ride summary for sharing.
- See status messages after ride actions.
- Prevent duplicate rides with the same route, date, and time.
...
- Clear all locally saved rides when you need a clean board.
...
Rides are saved in the current browser. Use the import and export controls in the footer to back up or move ride data between browsers. Export files include the export date in the filename and a small version marker for future compatibility.
